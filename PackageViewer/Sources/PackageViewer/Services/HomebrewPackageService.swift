...
class HomebrewPackageService: PackageRepository {
    ...
    func updatePackage(_ package: Package) async throws {
        // Use 600 seconds (10 minutes) timeout for brew upgrade which may need to download and compile packages
        // First try upgrading as a formula
        do {
            _ = try await shellService.execute(command: "brew", arguments: ["upgrade", package.name], timeout: 600.0)
        } catch {
            ...
            if casks.contains(package.name) {
                _ = try await shellService.execute(command: "brew", arguments: ["upgrade", "--cask", package.name], timeout: 600.0)
            } else {
                throw error
            }
        }
